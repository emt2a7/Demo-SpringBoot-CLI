export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // 變數準備區
      // ==========================================
      const GITHUB_TOKEN   = env.GITHUB_TOKEN;     // GitHub Token
      const LINE_TOKEN     = env.LINE_TOKEN;         // Line Token
      const TELEGRAM_TOKEN = env.TELEGRAM_TOKEN; // Telegram Token

      const GITHUB_REPO          = "emt2a7/Demo-SpringBoot-CLI"; // GitHub repo
      const GITHUB_WORKFLOW_NAME = "run-prod-agent.yml"; // GitHub Actions Workflow Name
      ...
      const path = url.pathname;                 // 解析 Request 網址後面的路徑
