...
public class Rag06Service {

    // 注入 自訂的 ExcelImageExtractor，負責從 excel 中挖圖並取得圖片實體路徑
    private final ExcelImageExtractor excelImageExtractor;
    ...
    public void updateBabyDocument(Resource wordResource) {
        ...
        log.info("上傳 excel 文件至 Baby 向量資料庫 (Vector Store): {}", fileName);
        ...
        try {
            ...
            // ==========================================
            // 解析 excel 文件圖片轉成文字描述
            imageDescriptionList = askExcelImageToDescription(wordResource);
     ...
     * @param excelResource
     */
    private ArrayList<String> askExcelImageToDescription(Resource excelResource) {
        var fileName = excelResource.getFilename();
        log.info("解析 excel 文件圖片轉成文字描述: {}", fileName);
        ...
        try {
            ...
            // 讀取 excel 檔案中所有圖片的實體路徑，並且按照「頁碼」對應到圖片清單。
            List<String> images = excelImageExtractor.extractImages(excelResource, outputImageDIr);
